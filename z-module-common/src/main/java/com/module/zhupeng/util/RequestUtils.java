...
public abstract class RequestUtils {
    ...
    public static boolean isApplicationJsonHeader(HttpServletRequest request) {
        return isApplicationJsonHeader(request.getHeader(HttpHeaders.CONTENT_TYPE));
    }

    public static boolean isApplicationJsonHeader(String contentType) {
        return contentType != null && StringUtils.replace(
                contentType.trim(),
                StringUtils.SPACE,
                StringUtils.EMPTY
    ...
    public static String getRequestJsonBody(NativeWebRequest webRequest) {

        String jsonBody = (String) webRequest.getAttribute("request", NativeWebRequest.SCOPE_REQUEST);
        if (jsonBody == null) {
            try {
                HttpServletRequest servletRequest = webRequest.getNativeRequest(HttpServletRequest.class);
                jsonBody = IOUtils.toString(servletRequest.getInputStream(), servletRequest.getCharacterEncoding());
                webRequest.setAttribute("request", jsonBody, NativeWebRequest.SCOPE_REQUEST);
            } catch (IOException e) {
     ...
     * 得到请求地址
     *
     * @return 如果是在http://www.google.com域名下请求当前服务器某个api,比如:http://www.digiwin.com/api,<p>
     * 那么获取到的就是http://www.google.com这个地址,而不是http://www.digiwin.com/api
     */
    ...
    public static String getRequestParameters(HttpServletRequest request) throws IOException {
        ...
        if (RequestUtils.isApplicationJsonHeader(request)) {
            //	 parameters.append(IOUtils.toString(request.getInputStream(), StandardCharsets.UTF_8.displayName()));Stri
     ...
     * @return 返回 /admin***
     */
    private static String getServletURI() {
     ...
     * @return 返回 /admin***
     */
    private static String getServletURI(HttpServletRequest request) {
